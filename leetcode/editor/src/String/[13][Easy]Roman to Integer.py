...
class Solution:
    def romanToInt(self, inputString: str) -> int:
        romanNumerals = {'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000}
        result = 0
        for i in range(len(inputString)):
            if i == len(inputString) - 1:
                result += romanNumerals.get(inputString[i])
            else:
                if romanNumerals.get(inputString[i]) < romanNumerals.get(inputString[i + 1]):
                    result -= romanNumerals.get(inputString[i])
                else:
                    result += romanNumerals.get(inputString[i])
        return result
...
class TestCase(unittest.TestCase):
    ...
    def testRomanToInt_numberMinusNumber(self):
        inputString = "IX"

        expectedResult = 9

        self.assertEqual(Solution().romanToInt(inputString), expectedResult)

    def testRomanToInt_numberPlusNumber(self):
        inputString = "LVIII"
    ...
    def testRomanToInt_plusAndMinus(self):
        inputString = "MCMXCIV"
